...
<div class="container">
    <div class="card">
        <div class="card-body">
            <h3 class="card-title text-center">Top 5 Most Active Transit Lines</h3>
            <%
                try {
                    ApplicationDB db = new ApplicationDB();
                    Connection con = db.getConnection();

                    // Query to find top 5 most active transit lines
                    String query = 
                        "SELECT tl.tl_name, COUNT(r.rid) AS tickets_booked " +
                        "FROM reservations r " +
                    ...
                    out.println("<thead><tr><th>Transit Line</th><th>Tickets Booked</th></tr></thead>");
                    out.println("<tbody>");
                    while (rs.next()) {
                        String transitLine = rs.getString("tl_name");
                        int ticketsBooked = rs.getInt("tickets_booked");
                        out.println("<tr>");
                        out.println("<td>" + transitLine + "</td>");
                        out.println("<td>" + ticketsBooked + "</td>");
                        out.println("</tr>");
                    }
                    out.println("</tbody>");
                    out.println("</table>");
                    ...
                    e.printStackTrace();
                    out.println("<div class='alert alert-danger'>Error fetching top transit lines.</div>");
                }
            %>
